...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompts", type=Path, default=Path("prompts/refinement_round33_hybrid_repair.json"))
    parser.add_argument("--targets", type=Path, default=DEFAULT_TARGET_DIR)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--identity", default="round33_multiseed")
...
def add_maxstack_scores(rows: list[dict[str, Any]]) -> None:
    ...
    for group in by_target.values():
        ...
        for row, region, heuristic, visual in zip(group, region_good, heuristic_good, visual_good):
            ...
            base_score = float(row.get("selection_score", row.get("combined_score", 0.0)))
            if row.get("target_name") == "9338.png":
                row["maxstack_score"] = (
                    0.50 * base_score +
                    0.16 * region +
                    0.22 * heuristic +
